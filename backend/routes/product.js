...
router.get('/getById/:id', auth.authenticateToken, (req, res, next) => {
    const id = req.params.id;
    var query = "select p.id,p.name,p.model,p.year,p.brand,p.description,p.price,sp.id as statusProductId,sp.name as statusProductName, ca.id as cateogoryId, ca.name as categoryName from product as p INNER JOIN statusProduct as sp, category as ca where p.statusProductId = sp.id and p.category = ca.id";
    connection.query(query, [id], (err, results) => {
        if (!err) {
...
router.patch('/update', auth.authenticateToken, checkRole.checkRole, (req, res, next) => {
    let client = req.body;
    var query = "update product set name=?,model=?,year=?,brand=?,description=?,price=?,statusProductId=? where id=?";
